...
FORCE_CUDA=1 pip install git+https://github.com/facebookresearch/pytorch3d.git # no ninja for stable build (but slower)
cd PoseEstimation/third_party/kaolin && FORCE_CUDA=1 python setup.py develop --user && cd ../..
